    ...
    .done(function (response) {
      ...
      if (eventCardInfo.length > 0) {
        eventCardInfo.forEach(function (cardItem) {
          ...
          cardDom.attr(
            'href',
            '/www/event-list/detail/index.html?event-id=' + cardItem.event_id
          )
          cardDom
            .find('img')
            .attr('src', 'http://192.168.137.1:8080' + cardItem.image)
          ...
          cardDom
            .find('.user-info img')
            .attr('src', 'http://192.168.137.1:8080/' + cardItem.icon)
          cardDom.show()
